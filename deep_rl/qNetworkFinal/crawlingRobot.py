...
class CrawlingRobot:
    ...
    def get_observation(self):
        raise NotImplementedError('Not implemented in CrawlingRobot class! Use CrawlingRobotDiscrete instead')
    ...
    def render(self):
        ...
        text1 = "Deep Reinforcement Learning for Crawling Robot"
    ...
    def plot_rewards(self):
        ...
        self.ax_rewards.grid(b=True, which='major', color='#666666', linestyle='-', alpha=0.2)
...
class CrawlingRobotDiscrete(CrawlingRobot):
    ...
    def action(self, action:int):
        ...
        if target_angle_upper == rounded_upper and target_angle_lower == rounded_lower:
            # skip action, if already in correct discrete target state
            return
        super().action([target_angle_upper, target_angle_lower])

    def get_observation(self):
        upper_arm_data = max(min(round((self.get_arm_angle(self.upper_arm) / self.ROTATION_ANGLE_STEP)), self.ROTATION_ANGLES - 1), 0)
        lower_arm_data = max(min(round((self.get_arm_angle(self.lower_arm) / self.ROTATION_ANGLE_STEP)), self.ROTATION_ANGLES - 1), 0)
        return [upper_arm_data, lower_arm_data]
